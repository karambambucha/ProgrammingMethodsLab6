...
namespace ProgrammingMethodsLab6
{
    internal class Rectangle
    {
		...
		public void BoxTest()
		{
			...
			FigureType = "Четырехугольник общего вида";
			if (sides[0] == sides[2] && sides[1] == sides[3])
			{
				FigureType = "Параллелограмм";
			...
			{
				FigureType = "Трапеция общего вида";
                if (sides[0] == sides[2] || sides[1] == sides[3])
                {
					FigureType = "Равнобедренная трапеция";
				}
                if (angles.Contains(90))
                {
					FigureType = "Прямоугольная трапеция";
				}
			}
		}
